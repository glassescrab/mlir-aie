...
int main(int argc, const char *argv[]) {
  ...
  constexpr int SMALL_TILE_ROWS = 8;
  constexpr int SMALL_TILE_COLS = 8;
  ...
  for (int large_tile_col = 0; large_tile_col < N / LARGE_TILE_SIZE; large_tile_col++) {
    for (int large_tile_row = 0; large_tile_row < K / LARGE_TILE_SIZE; large_tile_row++) {
      ...
      // Store weight data in small tiles (16x16 tiles)
      for (int small_tile_row = 0; small_tile_row < LARGE_TILE_SIZE / SMALL_TILE_ROWS; small_tile_row++) {
        for (int small_tile_col = 0; small_tile_col < LARGE_TILE_SIZE / SMALL_TILE_COLS; small_tile_col++) {
          int small_tile_index = small_tile_row * (LARGE_TILE_SIZE / SMALL_TILE_COLS) + small_tile_col;
          int small_tile_offset_bytes = small_tile_index * SMALL_TILE_ROWS * SMALL_TILE_COLS * sizeof(B_DATATYPE);
          
          for (int i = 0; i < SMALL_TILE_ROWS; i++) {
            for (int j = 0; j < SMALL_TILE_COLS; j++) {
              int weight_offset_bytes = large_tile_offset_bytes + small_tile_offset_bytes + (i * SMALL_TILE_COLS + j) * sizeof(B_DATATYPE);
              int matrix_row = large_tile_row * LARGE_TILE_SIZE + small_tile_row * SMALL_TILE_ROWS + i;
              int matrix_col = large_tile_col * LARGE_TILE_SIZE + small_tile_col * SMALL_TILE_COLS + j;
      ...
      // one scale per output column of the tile.
      for (int i = 0; i < LARGE_TILE_SIZE; i++) {
        int matrix_col = large_tile_col * LARGE_TILE_SIZE + i;
        if (matrix_col < N) {
  ...
  bvec_structured_csv << "# Small tiles: " << SMALL_TILE_ROWS << "x" << SMALL_TILE_COLS << " (" << (LARGE_TILE_SIZE/SMALL_TILE_ROWS) * (LARGE_TILE_SIZE/SMALL_TILE_COLS) << " per large tile)\n";
  ...
  for (int large_tile_col = 0; large_tile_col < N / LARGE_TILE_SIZE; large_tile_col++) {
    for (int large_tile_row = 0; large_tile_row < K / LARGE_TILE_SIZE; large_tile_row++) {
      ...
      // Print weights in small tiles (8x16 tiles)
      for (int small_tile_row = 0; small_tile_row < LARGE_TILE_SIZE / SMALL_TILE_ROWS; small_tile_row++) {
        for (int small_tile_col = 0; small_tile_col < LARGE_TILE_SIZE / SMALL_TILE_COLS; small_tile_col++) {
          int small_tile_index = small_tile_row * (LARGE_TILE_SIZE / SMALL_TILE_COLS) + small_tile_col;
          int small_tile_offset_bytes = small_tile_index * SMALL_TILE_ROWS * SMALL_TILE_COLS * sizeof(B_DATATYPE);
            ...
            for (int j = 0; j < SMALL_TILE_COLS; j++) {
              ...
              if (weight_offset_bytes < BVec.size()) {
                ...
                bvec_structured_csv << "weight," << large_tile_index << "," << small_tile_index 
                                   << "," << (i * SMALL_TILE_COLS + j) << "," << static_cast<int>(*weight_ptr) << "\n";
              }
            }
          }
        }
