...
def my_matmul(
    ...
    #   Weights (Int8): 128 * 64 = 8192 bytes
    #   Scales (BF16, one per output column, duplicated to fill the
    #   128-entry region for DMA alignment): 128 * 2 = 256 bytes
    # Total Bytes per Tile = 8192 + 256 = 8448 bytes.
    # The B buffer is shaped as memref<128 x new_n x i8>.
    # new_n = Total Bytes / 128 rows = 8448 / 128 = 66 = n + 2.
    new_n = n + 2
    n_aie_rows = 4
    ...
    def device_body():
        ...
        B_l2_ty = np.ndarray[(ktn * new_n,), np.dtype[dtype_in_B]]
                ...
                for pingpong in [0, 1]:
                    ...
                    for tile_row in range(tb_n_rows):
                        for col in range(n_aie_cols):
                            ...
                            B_col_offset = col * K * new_n * N // n // n_aie_cols
