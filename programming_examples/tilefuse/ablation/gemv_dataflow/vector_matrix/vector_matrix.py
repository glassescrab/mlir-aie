...
# (c) Copyright 2025 University of Illinois Urbana-Champaign.
import numpy as np
...
def my_matmul(dev, N, K):
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            # For GEVM: vector-matrix multiply (vector A, matrix B, output C)
            ...
            inA_fifos = [None] * n_cols
            outC_fifos = [[None] * n_cols for _ in range(n_rows)]
            inB_fifos = [[None] * n_cols for _ in range(n_rows)]
            ...
            for col in range(n_cols):
                ...
                # Input B (matrix - needs transformation from row-major to 32-bit word transposed)
                memB_fifos[col] = object_fifo(
                        ...
                        inB_ty,
                        # None,
                        # # S2MM in MemTile to convert n * nxk block from row-maj
                        # # to tiled k*n blocks (transposed at 32-bit word granularity)
                        # [
                        #     [
                        #         (k, n),
                        #         (nxk // n, k * n),
                ...
                for row in range(n_rows):
                    inB_fifos[row][col] = object_fifo(
                        ...
                        2, 
                        B_ty,
                        # MM2S pattern to read the transformed B tiles
                        # [
                        #     (nxk_div_n, k * n),
                        #     (n // 2, 2),
                        #     (k, n),
            ...
            def sequence(A, B, C):
                # GEVM: A is vector (K,), B is pre-tiled matrix (K, N), C is vector (N,)
                ...
                for j in range(num_iter):
                    for pingpong in [0,1]:
                        ...
                        for col in range(n_cols):
                            ...
                            B_base_offset = j * K * N // num_transfer * 2 + pingpong * K * N // num_transfer
                            B_col_offset = col * K * N // num_transfer // n_cols
                            ...
                            C_col_offset = col * N // num_transfer // n_cols
                            C_offset = C_base_offset + C_col_offset
                            C_size0 = C_sz_div_n_cols // num_transfer
                            npu_dma_memcpy_nd(
