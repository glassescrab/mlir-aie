...
int main(int argc, const char *argv[]) {
  ...
  // B matrix: int8 weights + 1 bfloat16 scale per output column (per-channel)
  ...
  std::vector<A_DATATYPE> BFactor_ref(N); // Per-output-channel scales: one per column of B
  ...
  // Change to custom ordering of large tiles
  ...
  for (int col_mod = 0; col_mod < 8; col_mod++) {
    for (int large_tile_col = col_mod; large_tile_col < N / LARGE_TILE_SIZE_COL; large_tile_col += 8) {
      for (int large_tile_row = 0; large_tile_row < K / LARGE_TILE_SIZE_ROW; large_tile_row++) {
        // Calculate byte offset for mixed data layout (128x64 tile + 128 scale factors)
        int large_tile_offset_bytes = large_tile_index * (LARGE_TILE_SIZE_ROW * LARGE_TILE_SIZE_COL * sizeof(B_DATATYPE) + LARGE_TILE_SIZE_ROW * sizeof(std::bfloat16_t));
          ...
          for (int small_tile_col = 0; small_tile_col < LARGE_TILE_SIZE_COL / SMALL_TILE_SIZE; small_tile_col++) {
            ...
            for (int i = 0; i < SMALL_TILE_SIZE; i++) {
              for (int j = 0; j < SMALL_TILE_SIZE; j++) {
                ...
                int matrix_row = large_tile_row * LARGE_TILE_SIZE_ROW + small_tile_row * SMALL_TILE_SIZE + i;
                int matrix_col = large_tile_col * LARGE_TILE_SIZE_COL + small_tile_col * SMALL_TILE_SIZE + j;
                ...
                if (matrix_row < K && matrix_col < N) {
                  ...
                  float scaled_weight = static_cast<float>(BVec_ref[ref_index]) / static_cast<float>(BFactor_ref[matrix_col]);
        ...
        // Store per-output-channel scale factors at the end of each large tile.
        // Only LARGE_TILE_SIZE_COL distinct scales exist per tile (one per output
        // column); they are written twice to fill the LARGE_TILE_SIZE_ROW-entry
        // region so the tile payload stays 128-byte aligned for the DMA. The
        // kernel reads only the first LARGE_TILE_SIZE_COL entries.
        for (int i = 0; i < LARGE_TILE_SIZE_ROW; i++) {
          int matrix_col = large_tile_col * LARGE_TILE_SIZE_COL + (i % LARGE_TILE_SIZE_COL);
          if (matrix_col < N) {
            int scale_offset_bytes = large_tile_offset_bytes + LARGE_TILE_SIZE_ROW * LARGE_TILE_SIZE_COL * sizeof(B_DATATYPE) + i * sizeof(std::bfloat16_t);
            std::bfloat16_t* scale_ptr = reinterpret_cast<std::bfloat16_t*>(BVec_bytes + scale_offset_bytes);
            *scale_ptr = static_cast<std::bfloat16_t>(BFactor_ref[matrix_col]);
          }
        }
        large_tile_index++;
      }
  ...
  bfactor_ref_csv << "# BFactor_ref (per-output-channel scales): " << N << " values\n";
  for (int i = 0; i < N; i++) {
  ...
  for (size_t i = 0; i < BVec.size(); i++) {
    ...
    if (i % (LARGE_TILE_SIZE_ROW * LARGE_TILE_SIZE_COL + LARGE_TILE_SIZE_ROW * sizeof(std::bfloat16_t)) < LARGE_TILE_SIZE_ROW * LARGE_TILE_SIZE_COL) {
      bvec_raw_csv << "weight";
    } else {
  ...
  bvec_structured_csv << "# Each large tile: " << LARGE_TILE_SIZE_ROW*LARGE_TILE_SIZE_COL << " weights + " << LARGE_TILE_SIZE_ROW << " scale factors\n";
  ...
  for (int large_tile_col = 0; large_tile_col < N / LARGE_TILE_SIZE_COL; large_tile_col++) {
    for (int large_tile_row = 0; large_tile_row < K / LARGE_TILE_SIZE_ROW; large_tile_row++) {
      int large_tile_index = large_tile_col * (K / LARGE_TILE_SIZE_ROW) + large_tile_row; // Column-major indexing
      int large_tile_offset_bytes = large_tile_index * (LARGE_TILE_SIZE_ROW * LARGE_TILE_SIZE_COL * sizeof(B_DATATYPE) + LARGE_TILE_SIZE_ROW * sizeof(std::bfloat16_t));
