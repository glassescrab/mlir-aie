...
def my_matmul(
    dev, M, K, N, m, k, n, dtype_in_str, dtype_in_B_str, dtype_out_str, b_col_maj, trace_size
):
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            b_ty = np.ndarray[(k, n), np.dtype[dtype_in_B]]
            c_ty = np.ndarray[(m, n), np.dtype[dtype_out]]
            q_ty = np.ndarray[(k,), np.dtype[dtype_in]]
            ...
            compute_tile3_col, compute_tile3_row = 0, 3
            compute_tile3 = tile(compute_tile3_col, compute_tile3_row)
            ...
            # Input Q
            # inQ = object_fifo("inQ", compute_tile3, compute_tile2, 4, q_ty,
            # initValues=[
            #     np.ones((k,), dtype=dtype_in),
            #     np.ones((k,), dtype=dtype_in),
            #     np.ones((k,), dtype=dtype_in),
            #     np.ones((k,), dtype=dtype_in)],
            # )
            ...
            # Compute tile 2
            @core(compute_tile2, f"mix_{m}x{k}x{n}.o")
            def core_body():
                for _ in range_(0xFFFFFFFF):
                    for _ in range_(tiles) if tiles > 1 else range(1):  # issue #1547
                        ...
                        for _ in (
                            ...
                            # elem_in_q = inQ.acquire(ObjectFifoPort.Consume, 1)
