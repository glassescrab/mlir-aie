...
def my_matmul(dev, M, K):
    ...
    # GEVM: A is vector (K,), B is matrix (K, M), C is vector (M,)
    A_sz = K
    B_sz = K * M
    C_sz = M 
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            for col in range(n_cols):
                ...
                object_fifo_link(memA_fifos[col], [inA_fifos[row][col] for row in range(n_rows)])
                ...
                # Input B (matrix - pre-tiled, no transformations needed)
                memB_fifos.append(
            ...
            for row in range(n_rows):
                for col in range(n_cols):
                # Compute tile col
                    @core(cores[row][col], f"vm_{m}x{k}.o")
                    def core_body():
            ...
            def sequence(A, B, C):
                # GEVM: A is vector (K,), B is pre-tiled matrix (K, M), C is vector (M,)
                ...
                for j in range(num_iter):
                    for pingpong in [0,1]:
                        ...
                        for col in range(n_cols):
                            ...
                            # Transfer pre-tiled matrix B chunks
                            B_base_offset = j * K * M // num_iter + pingpong * K * M // num_iter // 2
                            B_col_offset = col * K * M // num_iter // 2 // n_cols
                            B_offset = B_base_offset + B_col_offset
                            B_sizes = [1, K//k, n_rows * m, k]
                            B_strides = [0, k, K, 1]
                            npu_dma_memcpy_nd(
