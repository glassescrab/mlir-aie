...
void vecmat_vectorized(T_in_A *__restrict a, T_in_B *__restrict b,
  ...
  const T_in_B *__restrict pBzp_b = b + n * k / 2 + k * 2; // times 2 because bf16 scale factor is 2 bytes
  ...
  for (unsigned i = 0; i < colA; ++i) chess_prepare_for_pipelining chess_loop_range(8, ) {
    ...
    const aie::vector<T_in_B, 16> Bzp_row = aie::load_v<16>(pBzp_b + i * 16);
    const aie::vector<int8, size_B> Bzp = aie::transpose(Bzp_row.template grow_replicate<size_B>(), 8, 8);
