  ...
  if (g_counter == 0) {
    ...
    for (unsigned i = 0; i < colA; ++i) chess_prepare_for_pipelining chess_loop_range(4, ) {
      for (unsigned j = 0; j < colB; ++j) chess_flatten_loop {
        ...
        aie::vector<int8, 16> Bzp_row = aie::load_v<16>(pBzp_b + j * 16);
        aie::vector<int8, MMUL::size_B> Bzp = Bzp_row.template grow_replicate<MMUL::size_B>();

        // Dequantize: cast int8 to bf16 and apply scaling
        aie::vector<uint4, MMUL::size_B> B_uint4 = B_quantized.template cast_to<uint4>();
        aie::vector<uint8, MMUL::size_B> B_uint8 = B_uint4.template unpack();
        aie::vector<int8, MMUL::size_B> B_int8 = B_uint8.template cast_to<int8>();
        
        // Subtraction: Weighted Int8 - ZeroPoint Int8
        aie::vector<int8, MMUL::size_B> B_int8_zeroed = aie::sub(B_int8, Bzp);
        aie::vector<bfloat16, MMUL::size_B> B_float = aie::to_float<bfloat16>(B_int8_zeroed);
...
#define matmul_vectorized_c_func(ctype_in_A, mlir_type_in_A, ctype_in_B, mlir_type_in_B, ctype_out,            \
  ...
  void matmul_##mlir_type_in_A##_##mlir_type_out(ctype_in_A *a_in, ctype_in_B *b_in,         \
    ...
    matmul_vectorized_##r##x##s##x##t##_##mlir_type_in_A##_##mlir_type_out<      \
        DIM_M / DIV, DIM_K, DIM_N>(a_in, b_in, c_out);                               \
  }
...
#define matmul_scalar_c_func(ctype_in_A, mlir_type_in_A, ctype_in_B, mlir_type_in_B, ctype_out, mlir_type_out, \
  ...
  void matmul_scalar_##mlir_type_in_A##_##mlir_type_out(                         \
    ...
    matmul_scalar<ctype_in_A, ctype_in_B, ctype_out, DIM_M / DIV, DIM_K, DIM_N>(a_in, b_in,   \
                                                            c_out);            \
  }
