...
alignas(aie::vector_decl_align) static bfloat16 a_buf[64 * 128];
alignas(aie::vector_decl_align) static bfloat16 a_sf[64 * 128];
...
static inline void matmul_vectorized_2x2_mmul(const T_in_A *__restrict pA,
  ...
  for (unsigned i = 0; i < colA; ++i) chess_prepare_for_pipelining chess_loop_range(4, ) {
    for (unsigned j = 0; j < colB; ++j) chess_flatten_loop {
      // Load A for quantization
      aie::vector<bf16, MMUL::size_B> A_raw;
      ...
      A_raw = aie::load_v<MMUL::size_B>(pA + (i * rowA + j) * MMUL::size_B);
