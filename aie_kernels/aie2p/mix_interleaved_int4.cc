...
template <typename T_in_A, typename T_in_B, typename T_out, int rowA, int colA, int colB>
static inline void matmul_scalar(T_in_A *a, T_in_B *b, T_out *c) {
  event0();
  T_in_A factor = 2.5;
  for (int row = 0; row < rowA; row++) {
    for (int col = 0; col < colB; col++) {
      ...
      for (int i = 0; i < colA; i++) {
        running_sum += a[row * colA + i] * aie::to_float<T_out>(b[i * colB + col]) * factor;
      }
 ...
 * 	    |.
 */
template <typename T_in_A, typename T_in_B, typename T_out, unsigned rowA, unsigned colA,
          unsigned colB, unsigned r, unsigned s, unsigned t,
          bool b_row_maj = true>
static inline void matmul_vectorized_2x2_mmul(const T_in_A *__restrict pA,
                                              const T_in_B *__restrict pB,
                                              // const T_in_A *__restrict pQ,
                                              T_out *__restrict pC) {

  using MMUL = aie::mmul<r, s, t, T_in_A, T_in_A, accauto>;
      ...
      for (unsigned j = 0; j < colB; j += 2)
...
#endif
        {
          ...
          if constexpr (b_row_maj) {
            ...
            pBs_b = pB + colA * colB * MMUL::size_B;
            pBzp_b = pB + colA * colB * MMUL::size_B + colA * s * 4; // times 4 because bf16 scale factor is 2 bytes, int4 is half byte

          } else {
            pB1 = pB + (j * colA) * MMUL::size_B;
            pB2 = pB + ((j + 1) * colA) * MMUL::size_B;
            pBs_b = pB + colA * colB * MMUL::size_B; //can be wrong, b_col_maj not supported yet
            pBzp_b = pB + colA * colB * MMUL::size_B + colA * s * 4;
          }
          ...
          aie::vector<T_in_B, MMUL::size_B> B0_int4;
          aie::vector<T_in_B, MMUL::size_B> B1_int4;
          ...
          aie::vector<T_in_A, MMUL::size_B> B0_raw;
          aie::vector<T_in_A, MMUL::size_B> B1_raw;
          aie::vector<T_in_B, 32> Bs_row;
          aie::vector<T_in_B, 16> Bzp_row;
          aie::vector<T_in_A, 8> Bs_row_cast;
          aie::vector<int8, 8> Bzp_row_cast;

          aie::vector<T_in_A, MMUL::size_B> Bs;
          aie::vector<T_in_A, MMUL::size_B> Bzp;

          if constexpr (b_row_maj) {
            B0_int4 = aie::load_v<MMUL::size_B>(pB1);
            pB1 += MMUL::size_B * colB;
            B1_int4 = aie::load_v<MMUL::size_B>(pB2);
            pB2 += MMUL::size_B * colB;
          } else {
            B0_int4 = aie::transpose(aie::load_v<MMUL::size_B>(pB1), t, s);
            pB1 += MMUL::size_B;
            B1_int4 = aie::transpose(aie::load_v<MMUL::size_B>(pB2), t, s);
            pB2 += MMUL::size_B;
          }
          ...
          // Bzp = aie::broadcast<int8, MMUL::size_B>(0);
          Bzp = aie::transpose(Bzp_row_cast.template grow_replicate<MMUL::size_B>(), t, s);
          ...
          B0_raw = aie::to_float<T_in_A>(B0_int8_zeroed);
          B1_raw = aie::to_float<T_in_A>(B1_int8_zeroed);
          auto B0_acc = aie::mul(B0_raw, Bs);
          auto B1_acc = aie::mul(B1_raw, Bs);
          ...
          for (unsigned i = 1; i < colA; ++i)
...
#endif
            {
              ...
              pA2 += MMUL::size_A;
              if constexpr (b_row_maj) {
                B0_int = aie::load_v<MMUL::size_B>(pB1);
                pB1 += MMUL::size_B * colB;
                B1_int = aie::load_v<MMUL::size_B>(pB2);
                pB2 += MMUL::size_B * colB;
              } else {
                B0_int = aie::transpose(aie::load_v<MMUL::size_B>(pB1), t, s);
                pB1 += MMUL::size_B;
                B1_int = aie::transpose(aie::load_v<MMUL::size_B>(pB2), t, s);
                pB2 += MMUL::size_B;
              }
              Bs_row = aie::load_v<32>(pBs_b);
              pBs_b += 32;
...
matmul_vectorized_8x8x8_bf16_f32(const bfloat16 *__restrict pA,
  ...
  return matmul_vectorized_2x2_mmul<bfloat16, int8, float, (m / r), (k / s), (n / t),
...
#define combos(X) X(bfloat16, bf16, int8, i8, bfloat16, bf16, 8, 8, 8)
#endif

#ifdef bf16_f32_ONLY
#define combos(X) X(bfloat16, bf16, int8, i8, float, f32, 8, 8, 8)
#endif
...
#define matmul_vectorized_c_func(ctype_in_A, mlir_type_in_A, ctype_in_B, mlir_type_in_B, ctype_out,            \
                                 mlir_type_out, r, s, t)                       \
  void matmul_##mlir_type_in_A##_##mlir_type_out(ctype_in_A *a_in, ctype_in_B *b_in,         \
                                               ctype_out *c_out) {             \
    matmul_vectorized_##r##x##s##x##t##_##mlir_type_in_A##_##mlir_type_out<      \
        DIM_M, DIM_K, DIM_N>(a_in, b_in, c_out);                               \
  }

#define matmul_scalar_c_func(ctype_in_A, mlir_type_in_A, ctype_in_B, mlir_type_in_B, ctype_out, mlir_type_out, \
                             r, s, t)                                          \
  void matmul_scalar_##mlir_type_in_A##_##mlir_type_out(                         \
      ctype_in_A *a_in, ctype_in_B *b_in, ctype_out *c_out) {                      \
    matmul_scalar<ctype_in_A, ctype_in_B, ctype_out, DIM_M, DIM_K, DIM_N>(a_in, b_in,   \
                                                            c_out);            \
  }

#define zero_vectorized_c_func(ctype_in_A, mlir_type_in_A, ctype_in_B, mlir_type_in_B, ctype_out,              \
                               mlir_type_out, r, s, t)                         \
  void zero_##mlir_type_out(ctype_out *c_out) {                                \
    zero_vectorized<ctype_out, DIM_M, DIM_N>(c_out);                           \
  }

#define zero_scalar_c_func(ctype_in_A, mlir_type_in_A, ctype_in_B, mlir_type_in_B, ctype_out, mlir_type_out,   \
                           r, s, t)                                            \
